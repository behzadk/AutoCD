...
make_contour_plot <-function(x_data, y_data, x_lims, y_lims, weights_data, true_val_x, true_val_y) {
	...
	dens <- sm.density( cbind(x_data, y_data), hmult=0.5, weights=weights_data, display="none", nbins=0)
...
# print(colnames(data_df[, 4:ncol(data_df)]))
# quit()

param_lims <- make_param_lims_from_input(data_df[, 4:ncol(data_df)], param_priors_inputs_path, species_inputs_path)
# print(param_lims)
print(colnames(param_lims))
print(length(data_df))
print(colnames(data_df[, 4:ncol(data_df)))
weights <- rep(1, dim(data_df)[1])
